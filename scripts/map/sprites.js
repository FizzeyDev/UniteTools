...
(function () {
  ...
  /* Convert a map sprite ID (generated by data.js) back to the pokemonId
     format used by poke_data.json / damage-calc.
     data.js normalisation:
       replace(/mega-/g,"mega_") → replace(/\./g,"") → replace(/'/g,"")
       → replace(/ /g,"_") → replace(/-/g,"_")
     So almost everything becomes underscored, but poke_data.json keeps
     underscores only for: mr_mime, mewtwo_x, mewtwo_y, hooh (no separator).
     Everything else (mega-*, multi-word moves, etc.) uses hyphens.        */
  const UNDERSCORE_IDS = new Set([
    'mr_mime', 'mewtwo_x', 'mewtwo_y',
  ]);

  function mapIdToCalcId(id) {
    // IDs that legitimately use underscores in poke_data.json
    if (UNDERSCORE_IDS.has(id)) return id;
    // All other map IDs: underscores came from hyphens → restore them
  ...
  function onDuelPick(entry) {
    ...
    const atkCalcId = mapIdToCalcId(atk.id);
    const defCalcId = mapIdToCalcId(def.id);
    window.open(`damage-calc.html?atk=${encodeURIComponent(atkCalcId)}&def=${encodeURIComponent(defCalcId)}`, '_blank');
    stopDuel();
  }
